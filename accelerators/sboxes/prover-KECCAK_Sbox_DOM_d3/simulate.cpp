...
#include "cxxrtl_prover-keccak_sbox_dom_d3.h"
#include "manager.h"
...
int main(int argc, char *argv[]) {
    Configuration config(argc, argv, Configuration::CircuitType::ACCELERATOR, "prover-keccak_sbox_dom_d3");
    ...
    int XxDI = X0 ^ (X1 << 5) ^ (X2 << 10) ^ (X3 << 15);

    #ifndef CONST_EXEC
	    top.p_InputxDI.set<uint32_t>(XxDI);

	    Node& sec = symbol("sec", 'S', 5);
	    std::vector<Node*> ssec = manager.get_shares(sec, 4);
        top.p_InputxDI.setNode(&Concat(*ssec[0], *ssec[1], *ssec[2], *ssec[3]));
        top.p_ZxDI.setNode(&symbol("z", 'M', 30));
    #else
        top.p_InputxDI.set<uint32_t, true>(X);
    #endif

    manager.begin_measure();

    prepare_step(manager, top);
    //top.p_InputxDI.set<uint32_t, true>(0x0);
    // Propagate
    prepare_step(manager, top);
    ...
    Q0 = top.p_OutputxDO.get<uint32_t>() & 0x1F;
    Q1 = (top.p_OutputxDO.get<uint32_t>() >> 5) & 0x1F;
    Q2 = (top.p_OutputxDO.get<uint32_t>() >> 10) & 0x1F;
    Q3 = (top.p_OutputxDO.get<uint32_t>() >> 15) & 0x1F;
