...
namespace aes_herbst_unsecure_ns {
    ...
    cxxrtl::value<32>& SBoxPrime(cxxrtl::value<32>& e) {
        ...
        if (sboxp_access < 40)
            ...
            temp = circuit->memory_p_uRAM_2e_RAM[program->get_index("ram", "temp_sub")].shr(cxxrtl::value<32>{static_cast<uint32_t>(program->get_position("temp_sub")*8)}).trunc<8>().val();
        ...
        cxxrtl::value<8> sboxEq = circuit->memory_p_uRAM_2e_RAM[program->get_index("ram", "sbox") + (index.data[0] / 4)].shr(cxxrtl::value<32>{static_cast<uint32_t>((index.data[0]%4)*8)}).trunc<8>().val();
...
void aes_herbst_unsecure::init_implem(Manager& manager, cxxrtl_design::p_top& top) {
    ...
    for (uint32_t i = 0; i < 256; i++) {
        ...
        top.memory_p_uRAM_2e_RAM.update(this->get_index("ram", "sboxp") + lineIdx, preVal, mask);
    }
