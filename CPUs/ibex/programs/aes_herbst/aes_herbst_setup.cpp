...
namespace aes_herbst_ns {
    ...
    cxxrtl::value<32>& SBoxPrime(cxxrtl::value<32>& e) {
        ...
        if (sboxp_access < 40)
            ...
            temp = circuit->memory_p_u__ram_2e_u__ram_2e_mem[program->get_index("ram", "temp_sub")].trunc<8>().val();
        ...
        cxxrtl::value<8> sboxEq = circuit->memory_p_u__ram_2e_u__ram_2e_mem[program->get_index("ram", "sbox") + index.data[0]].trunc<8>().val();
...
void aes_herbst::init_implem(Manager& manager, cxxrtl_design::p_top& top) {
    ...
    for (uint32_t i = 0; i < 256; i++) {
        ...
        top.memory_p_u__ram_2e_u__ram_2e_mem.update(this->get_index("ram", "sboxp") + lineIdx, preVal, cxxrtl::value<32>(0x000000FFu));
    }
