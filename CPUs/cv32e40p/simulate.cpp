...
int main(int argc, char *argv[]) {
    ...
    Configuration config(argc, argv, Configuration::CircuitType::CPU, "cv32e40p");
    ...
    for (int i = 0; i < 25000; ++i) {
        ...
        if (top.p_u__top_2e_instr__rdata__i.curr.node->nature != CONST
            || top.p_u__top_2e_core__i_2e_cs__registers__i_2e_pc__id__i.curr.node->nature != CONST
        ) {
        ...
        if (program->symbols_["_hang"].addr == program->pc(top)) {
            std::cout << "Reached hang symbol, stopping cv32e40p." << std::endl;
            reached_end = true;
            break;
        }
    }
